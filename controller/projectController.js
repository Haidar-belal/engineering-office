const { Project, StageContractor, Stage, StageMaterial, Material, Database2, Bill, ProjectDocument, StageBill, CopyProject } = require('../models');
...
exports.getRejectedProjects = async (req, res, next) => {
    ...
    try {
        const project = await CopyProject.findAll({
            where: {
                finished: false,
                accepted: false,
                office_id: office_id
...
exports.updateProjectToAccepted = async (req, res, next) => {
    const { id } = req.params;
    try {
        const project = await CopyProject.findOne({
            where: {
                copy_project_id: id,
                accepted: null,
                finished: false
